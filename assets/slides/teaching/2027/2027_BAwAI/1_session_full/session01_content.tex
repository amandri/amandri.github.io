...
\item \key{Office hours:} feel free to (DO) come: B612.
...
$\hookrightarrow$ analytical tension (not clear causality).
...
$\hookrightarrow$ different student/AI workflows under a common finance assignment, material, time and prompt budget.
\end{alertblock}
}
...
\end{columns}
\vfill
\centering\Large \textcolor{CourseNavy}{Start now. We compare outputs in about 20'.}
}
...
\end{block}
\sourcewithref{Spoiler of the full sequence (three messages).}{S1-50}
}
...
\end{itemize}
\end{columns}
\begin{block}{The key for a good starting point:}
...
\LiveAndFullFrame{S1-11}{Working interpretation about revenues}{
\begin{center}
\small
\begin{tabular}{lrrrr}
\toprule
...
\LiveAndFullFrame{S1-13}{LLMs generate conditional text, not truth}{
\footnotesize
...
An answer can be grammatically convincing while misstating a source, skipping a number or inventing a causal story (e.g., check "AI allucination"). 
Language quality is not source fidelity.
\end{alertblock}
...
\textbf{Evidence:} information from the (restricted) source used to support a claim/calculation.\par\smallskip
...
\textbf{Delegation} -- decide whether/when/what to hand to AI.\par
...
\end{columns}
\smallrule
\key{Course map:} Session 1 is principally \textbf{augmentation}: delegate extraction and drafting, retain judgement and verification. \\
...
\scriptsize
\begin{block}{Being clear about criteria:}
...
\LiveAndFullFrame{S1-19}{Prompt trajectory: P0 through P4}{
\scriptsize
...
\LiveAndFullFrame{S1-22}{Missunderstanding? Examples, structure and roles}{
\footnotesize
...
2. AI supplies a source reference + rationale. \\
3. Analyst verifies source and decides about the inference.\\
...
\LiveAndFullFrame{S1-26}{An evidence matrix records evidence and especially limits}{
\scriptsize
...
Start a \textbf{different fresh chat}.\\
...
\begin{itemize}
\item Sector/subsector frames drivers, cyclicality, etc.
\item Products/services distinguish commodity-like supply from specified/service-intensive offers.
...
\item Profitability describes conversion of sales into profit.
\item Growth needs a driver-level reading.
\item Returns/credit require later calculations.
\end{itemize}
\end{block}
\end{columns}
